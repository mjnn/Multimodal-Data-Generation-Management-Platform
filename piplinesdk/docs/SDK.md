...
**小白入门（能力 + 产物一张表）**：[SDK-CAPABILITIES-AND-ARTIFACTS.md](./SDK-CAPABILITIES-AND-ARTIFACTS.md)
...
3. **（可选）帧级画框**：可插拔检测器在帧上检出物体，写入 `bboxes.jsonl`（旁路 `*_bbox.jpg`；不改原图）
...
| `clip_videos.jsonl` | 预览视频路径记录（plain） |
| `preview/` | 给人看的预览视频、音频、清单 |
| `run.json` | 本次运行的元信息（版本、完成步骤等） |

> 框以 `bboxes.jsonl` 为准；**不再**产出带框预览 MP4。入门总览见 [SDK-CAPABILITIES-AND-ARTIFACTS.md](./SDK-CAPABILITIES-AND-ARTIFACTS.md)。
...
| 编码 **plain** 预览 MP4 | 否 | `encode_preview_videos` / `encode` |
...
ROS1 录制文件 (.bag)
  ...
  · 声学频谱图（PNG）、Mel 特征
        ...
        ├─（可选）encode_preview → plain MP4（无框）
        ▼
  语音识别 → 得到转写文本
        │
        ├──────────────────┐
  ...
  labels.jsonl      fusion_embeddings.jsonl
        │
        ▼
  materialize_preview → preview/（manifest + plain 预览）
```
...
| `OMNI_VIDEO_VARIANT` | 否 | `auto` | Omni **帧**路径：`plain` / `bbox` / `auto`（视频预览恒为 plain MP4） |
...
| `encode_preview_videos(...)` | `encode` | **plain** 预览 MP4，回写 `clip_videos.jsonl` |
...
# ENCODE_BBOX 已废弃：不再产出 clip_preview_bbox_*.mp4
...
# → run_dir/bboxes.jsonl 、 preview/clip_preview_camera*.mp4
```
...
本地 HMI「管线执行参数」会把 `bbox_enabled` / 检测器等写入上述环境变量，worker 走 planner；总览/校核用 `bboxes.jsonl` 叠加框（不再依赖烧录 bbox MP4）。详见仓库 `project-management/acceptance/HMI-SDK-BBOX.md` / `HMI-BBOX-OVERLAY.md`。
