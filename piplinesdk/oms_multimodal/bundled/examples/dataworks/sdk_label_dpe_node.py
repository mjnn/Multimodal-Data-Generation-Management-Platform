# =============================================================================
# DataWorks PyODPS3：sdk_label（DPE UDF × label_clips，支持 batch items[]）
...
def _build_sdk_label_udf(
    *,
    ...
    def _sdk_label_row(row: pd.Series) -> dict[str, Any]:
        import os
...
def main() -> None:
    ...
    if exit_if_pipeline_idle(batch_ctx, node_name="sdk_label"):
        return
    ...
    configure_dpe_engine()
    print(
        f"sdk_label DPE batch_size={len(job_rows)} parallel={min(dpe_parallel, len(job_rows))} "
    ...
    label_udf = _build_sdk_label_udf(
        dpe_cpu=dpe_cpu,
        dpe_memory=dpe_memory,
        oss_mount_url=oss_mount_url,
        mount_path=mount_path,
    ...
    print_batch_summary("label", result, parallel=parallel)
