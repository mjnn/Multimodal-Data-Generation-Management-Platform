...
import { isOssPreviewableFile, OssFilePreviewPanel } from './OssFilePreviewPanel'
import { OssShortcutBar } from './OssShortcutBar'
...
type OssBrowserPanelProps = {
  /** When true, show reset button in the bucket card extra. */
...
export function OssBrowserPanel({ showTitleExtra = true }: OssBrowserPanelProps) {
  ...
  const showReset = showTitleExtra && testMode && canManageUsers(user?.roles)
  ...
  return (
    <div data-testid="oss-browser-panel">
      {info && (
      ...
      >
        ...
        <Typography.Text type="secondary" style={{ display: 'block', marginBottom: 12, fontSize: 12 }}>
          ...
          {previewFile ? <> · 点击文件名预览文本/JSON；右侧为内容摘要</> : null}
        </Typography.Text>
