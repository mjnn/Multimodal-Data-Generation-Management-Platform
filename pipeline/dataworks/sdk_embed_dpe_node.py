# =============================================================================
# DataWorks PyODPS3：sdk_embed（DPE UDF × embed_clips，支持 batch items[]）
...
def _build_sdk_embed_udf(
    *,
    ...
    def _sdk_embed_row(row: pd.Series) -> dict[str, Any]:
        import os
        ...
        from oms_multimodal import ClientConfig, McBackendConfig, OmsMultimodalClient, embed_clips
        ...
        mode = media_mode if media_mode in {"local", "oss", "auto"} else "local"
        oss_prefix = ""
        if mode in {"oss", "auto"}:
            oss_prefix = f"{str(row['run_relpath']).strip('/')}/"
...
def main() -> None:
    ...
    if exit_if_pipeline_idle(batch_ctx, node_name="sdk_embed"):
        return
    ...
    configure_dpe_engine()
    print(
        f"sdk_embed DPE batch_size={len(job_rows)} parallel={min(dpe_parallel, len(job_rows))} "
    ...
    embed_udf = _build_sdk_embed_udf(
        dpe_cpu=dpe_cpu,
        dpe_memory=dpe_memory,
        oss_mount_url=oss_mount_url,
        mount_path=mount_path,
    ...
    print_batch_summary("embed", result, parallel=parallel)
