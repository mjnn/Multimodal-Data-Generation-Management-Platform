...
description: >
  ...
  开跑绑定、产物 cache/lineage、内置 oms_cabin / audio_array_spec / ivi_ui_stub。
...
内置：`oms_cabin`、`ivi_ui_stub`（占位）、`audio_array_spec`（`stages.label.model=nvh_sem_ast`）。
...
下一工单方向：`UI-NVH-REVIEW-SAVE`（人工写回 L6）。
